...
object FBoundedPolymorphism extends App {
  ...
  /**
   * We want each override to return a specific override relating to the type i.e override ..... : List[Cat]/ List[Dog]
   *
   */

  // Solution 1 - naive - error pronee
  ...
  //  trait Animal[A <: Animal[A]] { // recursive type is called F-Bounded Polymorphism
  ...
  /**
   * FBP falls apart when dropping a class hierarchy one level
   * See example below
   */
  //  class Crocodile extends Animal[Dog] {
  //    override def breed: List[Animal[Dog]] = ??? // List[Dog] !!
